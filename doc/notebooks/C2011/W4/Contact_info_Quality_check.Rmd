...
MAIN_DATASET_PATH   <- r"(~\Workspace\21057906DatosContacto20221213.xlsx)"
...
Checks the address of the participants that do not have a "complete interview"
for any reason related to the address; the relevant codes are:

```{r address-related-codes}
result_codes |> filter(`Fieldwork result code` %in% ADDRESS_RELATED_CODES)
```

The following cases have one of the result codes listed above:

```{r}
contact_data                                                         |>
  right_join(fieldwork_data, by = c(Q0002 = "Household Id (Q0002)")) |>
  right_join(returned_mail_motives, by = c(Q0002 = "ID"))            |> # TODO: Delete if all interviews have to be considered
  filter(`Fieldwork result code` %in% ADDRESS_RELATED_CODES)         |>
  select(
    !!!ID_VARS,
    `Fieldwork result code`, `Fieldwork result`,
    `Return motive` = ASUNTO # TODO: Delete if all interviews have to be considered
  )
```

Contact information for these participants has not been provided (cases are not
in the contact information file). These cases need to be double checked.
