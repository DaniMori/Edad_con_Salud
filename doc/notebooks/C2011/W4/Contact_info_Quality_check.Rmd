...
wrong_dir_data <- contact_data                                        |>
  ...
  select(InstanceID, Q0002, !!address_sym, !!zipcode_sym, !!town_sym) |>
  arrange(Q0002)
...
these addresses need to be double checked:
...
for any reason related to the address (e.g. "empty home").

```{r}
interview_data                                                  |>
  transmute(q0007a_result_s3 = q0007a_result_s3 |> as_factor()) |>
  filter(q0007a_result_s3 != "Completed interview")
```

No interview has a status different from "Completed interview".
