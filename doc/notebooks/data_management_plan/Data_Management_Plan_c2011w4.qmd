...
DOC_MAIN_INFO <- tribble(
  ~name,                  ~value,
   "Responsible",          "",
   "Dissemination level",  "Confidential (CCOMS team only)",
...
RESPONSIBLE         <- ""
DISSEMINATION_LEVEL <- "Confidential (CCOMS team only)"
DOC_TYPE            <- 
KEYWORDS            <- c(
  "Edad con Salud",
  "Cohort 2011",
...
VERSION_HISTORY <- tibble(
  Authors                  = c("Daniel Morillo"),
  Contributors             = c(""),
  Reviewers                = c("Blanca Dolz"),
  `Version nº`             = c("0.0"),
  Name                     = c("Borrador inicial"),
  Date                     = c("25-11-2022"),
  `Change description`     = c("Primer borrador")
)
...
```{r version-history}
VERSION_HISTORY |> select(-(Authors:Reviewers))
```
