...
COMPLETE_DB_FILE_OLD <- file.path(COMPLETE_DB_DIR_OUT, COMPLETE_DB_FILENAME_OLD)
...
PRECOVID_DB_FILENAME_NEW <- glue("snapshot_{MIG_DATE}.dta")
PRECOVID_DB_FILE_NEW     <- file.path(
  PRECOVID_DB_DIR_OUT,
  PRECOVID_DB_FILENAME_NEW
)
PRECOVID_DB_FILE_OLD     <- file.path(
  PRECOVID_DB_DIR_OUT,
  PRECOVID_DB_FILENAME_OLD
)
...
<!-- # TODO: Archivo en versión SPSS (otra versión equivalente / qué hacer) ? -->
...
- `Bases de datos maestras Edad con Salud/Ola_3/Cohorte_2019/Submuestra_1_preconfinamiento/snapshot_[MIG_DATE].dta`
...
- `Bases de datos maestras Edad con Salud/Ola_3/Cohorte_2019/Submuestra_1_preconfinamiento/snapshot_`r EXAMPLE_MIG_DATE`.dta`


<!-- # TODO: Aprobar borrado de archivo en versión SPSS -->
Se borra el archivo de BDD de muestra completa en version Stata SPSS:
...
# Pre-COVID DB files:
unlink(PRECOVID_DB_FILE_OLD)
## TODO: Version in SPSS?
...
<!-- ## TODO: Cambiar por `saveold` para guardar en Stata 13 -->
...
- Responsables designados de mantenimiento de datos (<!-- # TODO: Por determinar -->)
...
<!-- TODO: Backups de permisos, o bien permisos en subcarpetas sólo -->

Los siguientes cambios deben hacerse manualmente antes de ejecutar el proceso
de migración, en la fecha y hora acordadas.

Se eliminan los permisos de escritura de todos los usuarios en las dos
carpetas de Edad con Salud:
...
```{r update-db-files-exec, ref.label="update-db-files"}
...
```{r update-db-dirs-exec, ref.label="update-db-dirs"}
## TODO: Delete chunk content
# Complete sample DB files:
...
```{r update-outcome-vars-exec, ref.label="update-outcome-vars"}
## TODO: Delete chunk content
# Complete sample DB files:
result <- file.rename(COMPLETE_DATASETS_DIR, DATASETS_DIR_OUT)
if (!result) {
...
```{r update-outcome-descdocs-exec, ref.label="update-outcome-descdocs"}
## TODO: Delete chunk content
# Complete sample DB files:
result <- file.rename(COMPLETE_DESCDOCS_DIR, DESCDOCS_DIR_OUT)
if (!result) {
...
<!-- # TODO: Completar subsecciones-->

### Cambios en permisos

- Restaurar permisos anteriores

- Revocar todos los permisos en subcarpetas "history"

### Notificación a usuarios dependencias a actualizar

- Comando `keep`
- Comando `keep` por `saveold` (con opción `, version(13)`).
